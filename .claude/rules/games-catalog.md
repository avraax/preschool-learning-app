...
- **Objects that restate a number already on screen were all removed** (owner 2026-08-01): Lær Tal's
  star/dot cluster under the numeral, and Sammenlign Tal's piles beside the numerals. The surviving
  counting visual is Plus/Minus's ten-frame, and it survives because it counts toward an answer that is
  NOT shown. Apply that test before adding any countable row: if the child can already read the number
  off the screen, the objects only let them skip the reading.
- **Tal Quiz is LISTEN-then-recognise, and shows NOTHING of the answer** (owner 2026-08-01): the
  ...
  digit-swap distractors test; countable solve-it work lives in Plus/Minus's ten-frames.
