...
- **The production SHAPE of the adult surface.** Five items are owner tools hidden in the App Store
  ...
  dev. Verified 2026-09-05 in both directions for all of them. (There used to be a sixth,
  `konto.syncNow`; it went with the whole Synkronisering section on 2026-09-19, so the devTool set is
  five.)
...
| 2026-09-20 | `8883b1e` | **FULL PASS before the 1.1 build — every phase at or BETTER than the 1.0 baseline.** 830 tests · lint 0 errors · tsc · build · context 47778/48000 · audit 1906 · schema both IN SYNC · shots 12 match (re-shot for 1.1) · desc in sync · selftest · smoke **27/0/0** · layout **216/0/0, 0 DEAD** · audio **26/0/1, zero reds first pass** · difficulty 16/0/4 · round 12/0/4 · ceremony **8/0, zero `board advanced` reds** · live **20/0/0**. Every N/A that disappeared vs 1.0 was `/ordleg/mic`, removed with the game. The two ceremony routes that survived isolation in the 1.0 cycle (`/english/listen`, `/math/comparison`) both passed. Ceremony 8-vs-9 is `/math/patterns` click budget, UNKNOWN not FAIL — see §2.1. All 3 lexemes (fire/fly/hund) verified APPLIED against a cache-busted fetch; `lexicon:check`'s NO EFFECT is Azure's lexicon cache. Rungs 1–2; §5 owed on build 18. |
| 2026-09-25 | `6511a46` | **Targeted pass over what 1.1 changed, beyond the row above.** Per-skin layout, which no earlier run had: `sweep.mjs --phase layout --theme` chrome ocean/space/dino **216/0/0 each**; webkit kid + space at `--vp iPadPro-land,ipad-land,phone-land,phone-port` **108/0/0 each**, 0 DEAD everywhere. **Every earlier "4 skins verified" capture is suspect**: `?theme=` under `?nogate=1` silently rendered the default skin until `6511a46`. By hand (chrome, guest, `?hidetools=1`, 1366×992 / 390×844 / 844×390): the gate refuses a wrong answer; Konto shows Google + Apple (prod `/family/providers` = both); Lyd = two switches; Privatliv AI disclosure; no Face ID or sync rows; Send feedback round-trips with æøå + screenshot; `/ordleg/mic` → NotFound, Ordleg = 2 games; the intro lands ~0.6s after the board, then title → pause → prompt, and a tap in the lead-in suppresses the title; `/api/usage` = one batch on hide, `credentials: omit`, unmapped routes dropped. **Found:** `appVersion` is `package.json`'s `1.0.45`, unchanged since June, so the counter and the adult footer cannot tell 1.1 from 1.0. Also, "Hør igen" in the first ~0.7s plays the prompt twice (pre-existing in 1.0). UNKNOWN: signed-in PIN/forgot-PIN (a local session writes to prod Neon), real sign-in, the shell. |
