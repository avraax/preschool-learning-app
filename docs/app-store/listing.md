...
• Ingen sporing og ingen analyseværktøjer fra tredjeparter
...
• No ads
• No in-app purchases
• No tracking and no third-party analytics
...
- **Advertising identifier (IDFA).** Answer **No.** The app has no ads, no attribution SDK and no
  third-party analytics (verified 2026-08-06: the apparent `amplitude` matches in this repo are the
  parallax token). The first-party anonymous counter added 2026-09-18 does not change this answer — it
  uses no identifier of any kind.
- **App Privacy questionnaire — MUST BE UPDATED (2026-09-18).** The anonymous usage counter
  (`docs/usage-analytics.md`) means the app now declares **Usage Data → Product Interaction**, *Not
  Linked to You*, **not** used for tracking, purpose **Analytics**. This mirrors the new entry in
  `ios/App/App/PrivacyInfo.xcprivacy`; keep the two in step. Apple allows these answers to be updated
  **without a new build and without App Review**, so it can be done before or after the build ships —
  but it must not be forgotten, because the store page would then contradict the binary's manifest.
