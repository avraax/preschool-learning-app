...
//
// Section *colors* (gradient/accent/border/hoverBorder) are now THEME TOKENS — they live in
// `src/theme/tokens/*` and are read here from the active skin. Only the *content/config* (name,
// description, and the games[] list with routes/titles/per-game button gradients) lives in this
// file. A reskin remaps colors; it never touches this content.
//
// A game's PICTURE is neither: it is theme-constant baked art in `src/assets/themes/icons/games/`,
// keyed `<section>.<game.id>`. De-emoji PRD-01 W4 deleted the `Game.emoji` fallback once every one
// of the 24 keys was confirmed to resolve — `gameIcons.test.ts` keeps that true.

import type { ThemeTokens } from '../theme/tokens/types'
import { getActiveTokens } from '../theme/tokens/activeTokens.ts'
import { kidThemeTokens } from '../theme/tokens/kidTheme.tokens.ts'
import { onTileColor } from '../theme/tokens/helpers.ts'
