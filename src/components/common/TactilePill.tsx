...
import { boxSoftShadow } from '../../theme/depth'
...
//
// Material: an accent-filled pill with a soft top sheen (the clay read), a grounded
// `boxSoftShadow()`, an inner-light highlight, and a small press-travel. No hard lip, no
// `backdrop-filter`. (The shadow was a chained `filter: drop-shadow()` pair until Performance PRD-01
// W4 — a pill is a box, so box-shadow draws the same picture without the filter passes.)
      ...
      sx={[
        {
          ...
          boxShadow: disabled
            ? 'none'
            : `inset 0 1px 0 rgba(255,255,255,0.4), ${boxSoftShadow(dark ? 1.4 : 1.1)}`,
          opacity: disabled ? 0.7 : 1,
          cursor: interactive ? 'pointer' : 'default',
          WebkitTapHighlightColor: 'transparent',
          outline: 'none',
          transition: 'transform 0.08s cubic-bezier(0.22,1,0.36,1), box-shadow 0.2s ease',
          ...
          ...(interactive
            ? {
                ...
                '@media (hover: hover) and (pointer: fine)': {
                  '&:hover': {
                    boxShadow: `inset 0 1px 0 rgba(255,255,255,0.4), ${boxSoftShadow(dark ? 1.9 : 1.6)}`,
                  },
                },
              }
            : {}),
