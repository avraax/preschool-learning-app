...
- **Games**: two patterns — task-based **quizzes** (bounded rounds of 8, global sticker pool → `RoundResultScreen`; wrong answers only break first-try) and calm **learning browses** (exploration-milestone stickers, no round). gameIds are `<section>.<game>`. Per-game mechanics + invariants: `.claude/rules/games-catalog.md` · how to build: `.claude/rules/game-development.md` · drag games: `.claude/rules/drag-and-drop.md`.
...
- **Progress / rewards** (Game Experience Overhaul foundation): `progressStore` singleton (`src/services/progressStore.ts`, localStorage key `bornelaering-progress`, versioned + private-mode-safe) is the single source of truth for persistent single-profile progress — collected stickers, per-game bests (longest streak / best stars / best count), lifetime stars. Read it via `useProgress()` (`src/hooks/useProgress.ts`, `useSyncExternalStore`). Task quizzes run **bounded rounds** (`useRound`, default 8 questions, no timer; wrong answers don't punish, only break a question's first-try flag) and end on `RoundResultScreen` (stars → "Ny rekord!" ribbon → streak → `StickerReveal` → replay/album/back). `UnifiedQuizGame` opts in via an optional `round` + `gameId` config (absent → endless behavior). It also carries a **never-fail hint** (PRD-05): set `hintAfterNWrong` and after that many wrong taps the correct `AnswerTile` pulses (reduced-motion → static accent glow ring), matching the hand-rolled color/spelling games; the wrongs already broke first-try so no extra star bookkeeping. An optional `speakCorrectFact(item)` speaks a completed fact on a correct answer instead of the tapped-item echo. **Result screen (PRD-09):** action buttons stay `pointer-events:none` until they animate in, and a tap anywhere **fast-forwards** the ceremony — done by remounting the beats under a keyed `<Fragment>` (framer won't reschedule an already-pending delayed animation just by lowering its `delay`); the star timeline collapses to the actual star count. Award lines (spoken summary + browse-milestone TTS) branch on `award.isNew` — a duplicate says **"skinnende"**, never **"nyt"**.
- **Progression / journey** (Liveliness PRD-01): purely-rewarding cross-game layer atop the sticker economy. `progressStore.progression` (schema **v2**) stores only `globalXp` + per-section `bloom` XP + `lastCelebratedLevel`; level and bloom stage/fill are **derived** via pure `src/config/progression.ts` (`levelFromXp`/`roundXp`/`bloomStage`), never stored. One play feeds both layers: `grantXp(section, n, reason)` (browse milestones) and XP folded into `recordRoundResult` (`outcome.xp`). Read via `useProgress()` (`globalLevel`/`xpProgress`/`bloomFor`). XP derives from round **structure only — never the difficulty setting** (fairness). Level-up: `levelUpBus.emit` → app-root `LevelUpOverlay` (`celebrateTier('levelup')` + `speakLevelUp`), with `LevelUpWatcher` as the reload/cross-tab safety net; fires once via `lastCelebratedLevel` (**starts at 1** so trin 1 isn't celebrated) → `markLevelCelebrated` on dismiss. Home shows `ProgressionCompanion` (ring + growing companion + level badge). New spoken praise is a closed set → prebaked + auditioned like other narration.
