...
- **Counting aid (Math)**: `CountingAid` (`src/components/common/CountingAid.tsx`) is the shared ten-frame manipulative — `mode="value"` (N dots, top row accent / bottom darker for subitizing), `mode="add"` (a accent + b second-colour dots), `mode="sub"` (a dots, last b crossed-out). It's revealed **on-demand** via the paired `TaelMedMigButton` ("Tæl med mig 🔢"), never auto-shown, so games stay a recognition test first / scaffold second. All Math games use it; `UnifiedQuizGame` exposes it via an optional `aidContent?: (item) => ReactNode` config. Math `gameId`s: `math.counting`, `math.numbers`, `math.addition`, `math.subtraction`, `math.comparison`, `math.patterns` (all bounded rounds of 8, global sticker pool). Sammenlign Tal is "tap the bigger number → krokodille >/< mouth eats it" (equality dropped); Lær Tal adds a "Find tallet" challenge round + session-local exploration-milestone stickers (every 12 distinct taps).
