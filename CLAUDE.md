...
- **Math games** (Game Experience Overhaul): all six run **bounded rounds of 8** with the global sticker pool. Math `gameId`s: `math.counting`, `math.numbers`, `math.addition`, `math.subtraction`, `math.comparison`, `math.patterns`. Tal Quiz / Plus / Minus use **near-number distractors** (digit-swap, off-by-one/ten) instead of random. Sammenlign Tal is "tap the bigger number → krokodille >/< mouth eats it" (equality dropped); Lær Tal adds a "Find tallet" challenge round + session-local exploration-milestone stickers (every 12 distinct taps).
