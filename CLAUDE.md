...
- **Audio**: Centralized `SimplifiedAudioController` singleton (single audio at a time — new audio cancels current; **no queue**). Audio unlocks on the **first gesture anywhere** — there is **no blocking permission modal**; the readiness verdict (`src/config/audioReadiness.ts`, pure: `idle | live | blocked`) is built only from signals that mean something actually sounded, and only `blocked` shows a small non-blocking "Tryk for lyd" chip. **`ctx.state === 'running'` is not liveness in either direction** (a probe context can sit `suspended` while narration plays, and WebKit 263627 reports `running` with a frozen clock) — liveness is a moving clock. See `.claude/rules/audio-system.md` for mandatory rules.
...
- **Animation mechanism & rendering cost**: a continuous, stateless animation is a **CSS keyframe animation** (`src/theme/idleMotion.ts`), NEVER a framer `repeat: Infinity` loop — 25 of those spent ~40% of every second recalculating style while the app sat still. Framer keeps one-shot feedback, gestures, `AnimatePresence`. A running CSS animation **outranks framer's inline `transform`**, so the two go on nested layers unless mutually exclusive by construction. `will-change` is spent, not sprinkled. **`content-visibility` is Safari 18 → banned** (silent on the floor device — the Ogg shape of mistake). "Flydende grafik" is a PERMANENT adult toggle switching the profile from **one branch point** (`src/config/perfProfile.ts`), and may change only the MECHANISM, never a layout. Guarded by `theme/idleMotionBudget.test.ts` + `config/perfProfile.test.ts`. **The `drop-shadow`-on-a-translucent-surface trap, the parallax driver, and how to measure any of this: `.claude/rules/animation-and-performance.md`.**
...
**"It stutters" is a STEADY-STATE question, and `cdp.mjs --perf` cannot see it** — that measures LOAD, and
its frame times are a software-raster artifact. Use `perf.mjs` (rung 1) for an app sitting still, and gate
on `recalcMsPerSec`, never `recalcPerSec` (Blink counts one recalc per animating frame whatever the
mechanism, so it never drops below ~60 while anything animates).
