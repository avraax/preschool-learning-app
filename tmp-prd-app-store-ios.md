...
| ~~**Two stale screenshots**~~ | **Ten of twelve RE-SHOT 2026-09-04** (`8f9621d`) — every child-facing shot, iPad 2732×2048 + iPhone 2868×1320, colour type 2. What remains is the **`voksne` pair**, and the decision on them is: **take the iPad one on the iPad** (owner, natively 2732×2048 from the TestFlight build) and **drop the iPhone one entirely** — a 5-shot iPhone set is valid, and the alternative is shipping a picture of a floating gear that no longer exists. The harness cannot take either: it reaches the arithmetic gate reliably but crashes WebKit the moment the gate is *solved*. See the re-capture hazard below. |
